...
# --- MAP (OPTIMIZED WITH PREFER_CANVAS) ---
m = leafmap.Map(
    ...
    prefer_canvas=True  # PERFORMANCE BOOST: Renders as Canvas instead of SVG
)

m.add_basemap(leafmap_basemaps[basemap_selection])

# Minimap forced to Lightmap
...
def add_layer(df, color, icon, label, is_met=False):
    ...
    # Use FeatureGroup for non-clustered markers for better speed
    ...
    for _, row in data.iterrows():
        ...
        # OPTIMIZED RADIUS: No stroke, very low opacity for blending
        ...
        if marker_style == "Classic (FontAwesome)":
            ...
            folium.CircleMarker(
                [row['lat'], row['lon']], radius=marker_size,
                popup=popup, tooltip=tooltip,
                color=color, fill=False, weight=2
            ).add_to(container)
            ...
            folium.CircleMarker(
                [row['lat'], row['lon']], radius=max(3, marker_size // 2),
                popup=popup, tooltip=tooltip,
                color=color, fill=True, fill_color=color, fill_opacity=1, weight=0
            ).add_to(container)
...
# --- RENDER ---
map_data = st_folium(
    m, width="100%", height=700, key="vn_system_pro",
